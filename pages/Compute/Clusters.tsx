...
const ClustersPage: React.FC = () => {
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      case 'healthy': return { text: '健康', color: 'text-emerald-600', dot: 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' };
      case 'degraded': return { text: '警告', color: 'text-amber-600', dot: 'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]' };
      case 'unhealthy': return { text: '异常', color: 'text-red-600', dot: 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]' };
      default: return { text: '未知', color: 'text-slate-400', dot: 'bg-slate-300' };
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20 font-sans">
      <RegisterModelModal isOpen={false} onClose={() => {}} /> {/* Hidden/Dummy component reference if needed by context */}
      ...
      <PageHeader 
        icon={Server}
        title="集群资源中心"
        subtitle="INFRASTRUCTURE ASSET MANAGEMENT"
        badgeText="INFRASTRUCTURE READY"
        actions={
          <>
            <CustomSelect
              options={[
              ...
              className="w-44"
            />
            <button onClick={() => window.location.reload()} className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-400 hover:text-primary-600 transition-all">
              <RefreshCw size={18} />
            </button>
            <button 
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/30 text-slate-400 border-b border-slate-100">
              ...
              <th className="px-8 py-4 text-[10px] font-black uppercase tracking-[0.2em]">集群状态</th>
                  ...
                  >
                    ...
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className={`text-[11px] font-black uppercase tracking-widest ${statusInfo.color}`}>{statusInfo.text}</span>
                      </div>
                    </td>
...
// 模拟图标占位，补充缺失组件引用
const RegisterModelModal = (props: any) => null;
